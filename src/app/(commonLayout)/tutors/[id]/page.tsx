  ...
  useEffect(() => {
    const loadTutorDetails = async () => {
      ...
      try {
        ...
        const tutorResult: ApiResponse<TutorProfile> = await tutorResponse.json()
        const reviewsResult: ApiResponse<TutorReview[]> = await reviewsResponse.json()

        if (!tutorResponse.ok) {
          throw new Error(tutorResult.message || 'Failed to load tutor details')
        }
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: result.message || `Your session with ${tutor.user.name} has been booked.`,
        confirmButtonColor: '#0f766e',
      })
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[linear-gradient(180deg,_#f8fafc_0%,_#f0fdfa_100%)] px-4">
        <div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-semibold text-slate-900">Tutor not found</h1>
          <p className="mt-3 text-sm text-slate-500">
            We could not find the tutor profile you requested.
          </p>
          <Button asChild className="mt-6 rounded-2xl bg-teal-600 text-white hover:bg-teal-700">
            <Link href="/tutors">
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{tutor.totalReviews} reviews</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200">
                      ...
                      <div>
                        <p className="text-lg font-semibold text-slate-900">${tutor.hourlyRate}/hour</p>
                        <p className="text-xs text-slate-500">Flexible session pricing</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200">
                      ...
                      <div>
                        <p className="text-lg font-semibold text-slate-900">{tutor.experience} years</p>
              ...
              <div className="rounded-[32px] border border-white/70 bg-white/85 p-6 shadow-xl shadow-teal-100/60 backdrop-blur">
                <h2 className="text-lg font-semibold text-slate-900">About this tutor</h2>
            ...
            <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-xl shadow-teal-100/70 backdrop-blur">
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="rounded-2xl bg-amber-50 p-4">
                  ...
                  <p className="mt-2 text-sm font-medium leading-6 text-slate-700">
                    Students looking for one-to-one support, flexible scheduling, and a clear lesson plan.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_380px] lg:px-8">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              {tutor.availability?.length ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                  This tutor has not added availability yet. You can still contact them or check back later.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Reviews loaded from `GET /review/tutor/:tutorProfileId`.
                </p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-3 text-center">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Average</p>
                <p className="mt-1 text-2xl font-bold text-slate-900">{tutor.avgRating.toFixed(1)}</p>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-4">
              {reviews.length ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                  No reviews yet. Once students complete sessions and leave feedback, reviews will appear here.
                </div>
              )}
            </div>
          </div>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Students can book directly with the backend `POST /booking` route.
            </p>
            ...
            <div className="mt-6 space-y-5">
              ...
              <div className="rounded-3xl bg-[linear-gradient(135deg,_#0f766e,_#0891b2)] p-5 text-white">
                <p className="text-xs uppercase tracking-[0.2em] text-teal-100">Estimated total</p>
                <p className="mt-2 text-3xl font-bold">${estimatedPrice.toFixed(2)}</p>
                <p className="mt-2 text-sm text-teal-50/90">
                  Based on ${tutor.hourlyRate}/hour for {bookingForm.duration} minutes.
                </p>
              </div>
              ...
              <p className="text-xs leading-6 text-slate-500">
                Booking is available for signed-in students. Tutors and admins can view this page, but cannot create bookings.
              </p>
            </div>
          </div>
        </aside>
